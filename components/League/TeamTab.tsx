...
const TeamTab = ({ league, participant, wallets, position }: { league: any, participant: any, wallets: any, position: any }) => { 
  ...
  return (
    ...
    <div className={`${L.resultsRow}`}>
        <span className="text-sm">{position}</span>  <span className="text-sm">{participant.username}</span> <span className="text-sm">{participant.points}</span> <span className="text-sm">${wallets.filter((wallet:any) => wallet.userId === participant.userId).map((wallet:any) => Intl.NumberFormat('en-US').format(wallet.credits))
